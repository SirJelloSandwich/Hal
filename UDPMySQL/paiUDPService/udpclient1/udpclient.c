...
void _INIT udpclientINIT(void)
{
	...
	udpClientInit(&gUdpClientMySQL);


	//upd client application specific init
	gUdpClientMySQL.clientIf.iConnect = 1;//connect
	gUdpClientMySQL.clientIf.iMode = 0;//echo

	//RESTORE SETTINGS FROM THE REMANENT STORAGE
	//sanity check for remote host ip address
	gUdpClientMySQL.clientIf.iRmtUdpPort = gUdpSystemCfg.udpRemoteHostPortNumber;

	strcpy(gUdpClientMySQL.clientIf.iRmtIPAddr, "192.168.2.105");
}
...
void _CYCLIC udpclientCYCLIC(void)
{
	...
	//udp client cyclic
	udpClientCyclic(&gUdpClientMySQL);


	//in both modes use configured port number for the remote UDP host
	gUdpClientMySQL.clientIf.iRmtUdpPort = gUdpClientMySQL_DefaultPort;
